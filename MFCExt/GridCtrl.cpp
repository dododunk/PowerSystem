...
void CGridCtrl::OnBeginPrinting(CDC *pDC, CPrintInfo *pInfo)
{
    ...
    int nFontSize = -12;//-10;//－14
    CString strFontName = "宋体";//"Arial";//
...
void CGridCtrl::PrintHeader(CDC *pDC, CPrintInfo *pInfo)
{
  ...
  // print parent window title in the centre (Gert Rijs)
	...
	*/
	...
	pDC->MoveTo(rc.left, rc.bottom + 12);//+14
    pDC->LineTo(rc.right, rc.bottom + 12);
}
...
void CGridCtrl::PrintFooter(CDC *pDC, CPrintInfo *pInfo)
{
	...
	m_pagefooter.TrimRight();
    m_pagefooter.TrimLeft();

    m_timefooter.TrimRight();
    m_timefooter.TrimLeft();
 
	// page numbering on left
	strLeft.Format(_T("第%d页,总计%d页"), pInfo->m_nCurPage, pInfo->GetMaxPage());
	m_pagefooter = strLeft;

    
	// date and time on the right
	COleDateTime t = COleDateTime::GetCurrentTime();
	if(m_timefooter.IsEmpty())
	{
        strRight     = t.Format(_T("%c"));
		m_timefooter = strRight;
	}
	else
	{
		strRight     = m_timefooter;
	}
...
BOOL CGridCtrl::Save(LPCTSTR filename, TCHAR chSeparator/*=_T(',')*/)
{
    ...
    TRY
    {
        ...
        for (int i = 0; i < GetRowCount(); i++)
        {
		 ...
		 File.WriteString(m_pagefooter);
	     File.WriteString("\r\n");

		 File.WriteString(m_timefooter);
         File.WriteString("\r\n");
