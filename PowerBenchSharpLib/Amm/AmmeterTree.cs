...
namespace PowerBenchSharpLib
{
    ...
    public class AmmeterTree : System.Windows.Forms.TreeView
    {
        ...
        {   
            ...
            {
                getLeafNode(item as TreeNode, _info);
            }
            return _info;
        }
        ...
        void AmmeterTree_NodeMouseClick(object sender, TreeNodeMouseClickEventArgs e)
        {
            ...
            if (e.Node.Tag is P_AmmeterInfo)
            {
                MenuItem menuItem_CloneAdd = new MenuItem("克隆添加");
                menuItem_CloneAdd.Click += menuItem_CloneAdd_Click;
                menu.MenuItems.Add(menuItem_CloneAdd);
            }
                        ...
                        foreach (var line in lines)//3-线
                        {
                            ...
                            foreach (var tranformer in transfromers)//4-变压器
                            {
                                ...
                                foreach (var module in modules)//5-模块
                                {
                                    ...
                                    var subammeters = from amm in module group amm by amm.USER_SUBAMM.ToString() into subammeter orderby subammeter.Key select subammeter;

                                    foreach (var subammeter in subammeters)//6-子表
                                    {
                                       // if (subammeter.Count() > 1)
                                        //{
                                          //  string msg = string.Format("存在重复的数据 :\r\n{0}", subammeter.First().ToString());
                                            //MessageBox.Show(msg, "错误");
                                    //    }
                                        foreach (var item in subammeter)
                                        {
